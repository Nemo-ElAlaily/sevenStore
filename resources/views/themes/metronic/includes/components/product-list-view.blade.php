            ...
            <li class="product list-view">
                <div class="media">
                    <div class="media-left">
                        <a href="#">
                            <img class="wp-post-image" data-echo="{{ $product->image_path }}"
...
{{--                                    </div>--}}
                            ...
                            <div class="col-xs-12">
                                ...
                                <div class="availability in-stock">
                                    {{ trans('front.Availability') }}:
                                    @if ($product->stock == 0)
                                        <span class="text-danger"> {{ trans('front.Out Of Stock') }}</span>
                                    @elseif ($product -> stock > 0 && $product -> stock < 5) <span class=""> {{ trans('front.Low Stock') }}</span>
                                        @else
                                ...
                                <a class="button product_type_simple add_to_cart_button ajax_add_to_cart"
                                    data-product_sku="{{ $product->sku }}" data-product_id="{{ $product->id }}"
                                    data-quantity="1" href="#" rel="nofollow">{{ trans('front.add to cart') }}</a>
