        ...
        @if(Cart::instance('cart')->count() > 0)
            @foreach(Cart::instance('cart')->content() as $item)
                ...
                <td class="product-thumbnail">
                    <a href="#"><img width="180" height="180" src="{{ $item -> model -> image_path }}" alt="{{ $item -> model -> slug }}"></a>
                </td>

                <td data-title="Product" class="product-name">
                    <a href="{{ route('front.product.details', $item -> model -> slug) }}">{{ $item -> model -> name }}</a>
                </td>
                ...
                <td data-title="Quantity" class="product-quantity">
                    <div class="quantity buttons_added">
                        ...
                        <input type="number" size="4" class="input-text qty text" title="Qty" value="{{ $item -> qty }}" name="{{ $item -> model -> qty }}" max="29" min="0" step="1">
                        <a class="btn btn-increase plus"  href="#" wire:click.prevent="increaseQty('{{ $item -> rowId }}')">+</a>
                        <a class="btn btn-reduce minus" href="#" wire:click.prevent="decreaseQty('{{ $item -> rowId }}')">-</a>
                    </div>
                </td>
